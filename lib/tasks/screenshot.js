...
async function generateThumbnails (options) {
  ...
  const allScreenshots = dirs.map(async (dir) => {
    ...
    const htmlFiles = files.filter((file) => {
      const matchFileName = (options.deploying && options.clm.crm.deploy_to === 'vault') ? 'index' : dir;
      return file.match(matchFileName + '.html');
    });

    if (htmlFiles.length === 0) {
      utils.log.warn(`No matching HTML file found to screenshot for key message "${dir}"`);
    }
...
async function screenshotsTask (options) {
  ...
  for (const km of options.clm.key_messages) {
    const keyMessage = utils.formatKeyMessage(options, km);
