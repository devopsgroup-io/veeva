...
helpers.showAllHelpers = function(context) {
  const sortedHelpers = Object.entries(Handlebars.helpers).sort(([a], [b]) => a.localeCompare(b));

  console.log('Registered Handlebars helpers:');
  // Format for console.table()
  const tableData = sortedHelpers.map(([name, fn]) => ({
    Name: name
  }));
