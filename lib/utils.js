...
module.exports = {
  formatKeyMessage: function (options, km) {
    if (options.clm.product && options.clm.product.name) {
      return (options.clm.product.name + options.clm.product.suffix + km.key_message).toLowerCase();
    }
    return km.key_message.toLowerCase();
  },
